function [pr9_model1,errors_of_training,errors_of_test] = pr9_ann_model_function(y,num_of_hidden_layer)
...
errors_of_training=[mean(error_training) max(error_training) bigger_than_five(2)];


ann=sim(pr9_model1,inputTestNew);
error_test=abs((ann-outputTestNew)./ann)*100;
bigger_than_five=size(find(error_test>5));
errors_of_test=[mean(error_test) max(error_test) bigger_than_five(2)];
